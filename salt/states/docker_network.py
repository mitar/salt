    ...
    if network is not None:
        ...
        try:
            ...
            def is_default_pool(x):
                # A pool is a "default" (auto-assigned) pool if it has only
                # a Subnet and Gateway set. Docker's inspect output can
                # include additional keys such as ``IPRange`` or
                # ``AuxiliaryAddresses`` with empty values; those are not
                # user-configured settings and must not disqualify the pool
                # from being treated as default.
                non_empty_keys = sorted(k for k, v in x.items() if v)
                return non_empty_keys == ["Gateway", "Subnet"]

            if (
                desired_pool_count == 0
                and existing_pool_count >= 1
                and all(
                    is_default_pool(pool) for pool in network["IPAM"]["Config"]
                )
            ):
